...
## 2026-08-28 - Cleaned up malformed AI code and Optimized Combobox Search
**Learning:** Found an extremely malformed code file (`SettingsModal.tsx`) containing dozens of duplicated `useMemo` attempts (with varied syntax and nested `try-catch`) likely left over from previously failing optimization attempts. Removing these fixed a massive codebase eyesore. The actual optimization—pre-computing lowercase static arrays (e.g. `tz.toLowerCase()`) into objects and hoisting the `.toLowerCase()` call for the dynamic search term outside the `.filter` loop—prevents O(N) string allocations during rapid typing.
**Action:** Always fully read the surrounding code block using exact line bounds (via Python or carefully targeted `sed`) before using Git Merge Diff. If a file is heavily corrupted by previous incomplete tool executions, clean it up as part of the PR. When restructuring data for performance, ensure the new array object maps back flawlessly to what the downstream JSX requires.
