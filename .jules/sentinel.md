...
## 2024-08-04 - [Server Action IDOR via Fallback Parameter]
**Vulnerability:** A critical vulnerability existed in `src/ai/actions/db-admin.ts` where the `verifyAdmin` function, which is responsible for authenticating and authorizing admin server actions, incorrectly accepted an `adminUserId` parameter as a fallback when the `nutrisnap_session_id` cookie was missing. This parameter was passed from the client in `src/app/admin/page.tsx`. This allowed an unauthenticated attacker to bypass authentication and execute any administrative action (creating, updating, deleting users, or viewing all users) simply by calling the server action directly with a valid or arbitrary `adminUserId` argument.
**Learning:** Next.js Server Actions are public endpoints. Trusting arguments provided by the client for authentication or authorization decisions is a critical flaw. Authentication must strictly rely on secure, server-side context, such as HttpOnly cookies validated against a database.
**Prevention:** Never use client-provided arguments as a fallback for authentication in Server Actions. Always explicitly extract and validate authentication state (e.g., via cookies) *inside* the Server Action. Remove any parameters from Server Actions that attempt to pass authentication context from the client.

## 2026-07-21 - [Information Leakage via Webhook Error Responses]
...
## 2026-07-22 - [User Enumeration Timing Attack via Authentication Endpoint]
**Vulnerability:** A High-priority User Enumeration timing attack vulnerability existed in `src/app/api/auth/login/route.ts`. When a user attempted to log in with an invalid email, the application immediately returned an error response without performing the computationally expensive bcrypt password hash comparison. This significant difference in response times allowed attackers to determine whether a given email address existed in the database.
**Learning:** Returning early from an authentication endpoint when a user is not found, without performing the same computationally expensive operations (like password hashing) as a successful user lookup, creates a detectable timing discrepancy that leaks user existence information.
**Prevention:** Always perform a dummy hash comparison (e.g., using a pre-computed dummy hash at the module level) when a user lookup fails, ensuring that the response time remains consistent regardless of whether the user exists or not.
