...
\n## 2025-02-14 - [Timing Attack] User Enumeration via Login Timing\n**Vulnerability:** The login API and authentication server actions returned early if a user was not found by email, bypassing the expensive `bcrypt.compare` operation. This measurable time difference allowed attackers to enumerate valid user emails.\n**Learning:** In custom authentication implementations using ORMs like Prisma, early returns before cryptographic operations create timing side-channels.\n**Prevention:** Always ensure a constant-time execution path for authentication failures by performing a dummy hash comparison (e.g., `bcrypt.compare(password, dummyHash)`) even when the user record does not exist.
