...
\n## 2026-08-25 - [User Enumeration Timing Attack via Authentication Failures]\n**Vulnerability:** A HIGH priority vulnerability existed in `src/app/api/auth/login/route.ts` and `src/ai/actions/db-users.ts` where authentication attempts that failed to find a matching user returned immediately, while successful lookups proceeded to perform an expensive `bcrypt.compare` operation. This measurable timing difference allowed attackers to confirm whether an email address existed in the database (User Enumeration).\n**Learning:** Next.js API Routes and Server Actions must execute dummy cryptographic operations when a resource lookup fails to ensure consistent response times and prevent timing-based enumeration attacks. However, one must never hardcode a valid hash directly in the source code as SAST tools flag it as a hardcoded secret. \n**Prevention:** When a user lookup returns null, execute a dummy hash comparison (`bcrypt.compare`) on a valid but dynamically generated or safe string, or conditionally only perform it if the password is provided, to mitigate timing differences.
