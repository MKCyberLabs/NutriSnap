...
function EditGramsPopover({
  ...
  item: FoodItem;
  logId: string;
  onUpdate: (logId: string, itemId: string, newGramsStr: string) => void;
}) {
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <button aria-label="Edit weight" className="flex items-center gap-1 text-[10px] text-muted-foreground font-bold hover:text-primary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm">
...
function AddItemPopover({
  ...
  logId: string;
  onAdd: (logId: string, text: string) => Promise<void>;
  isAdding: boolean;
}) {
...
function calculateNutrientTargets(user?: any) {
  ...
  const activeMetrics = user?.metrics || {
    height: 175,
    weight: 70,
    age: 30,
    gender: "male",
  ...
  return {
    ...
    protein: user?.dailyProteinGoal || Math.round((tdee * 0.3) / 4),
    carbs: user?.dailyCarbsGoal || Math.round((tdee * 0.4) / 4),
    fat: user?.dailyFatGoal || Math.round((tdee * 0.3) / 9),
    sugar: 35,
  };
}
...
export default function DashboardPage() {
  ...
  const handleMealCardComplete = useCallback(
    async (
      ...
      const isoTimestamp = isValid(logTimestamp)
      ...
      if (res && res.success && res.log) {
        setLogs((prev) => {
          const updated = prev.map((l) =>
            l.id === newLog.id ? { ...l, id: res.log.id, isPending: false } : l,
          );
  ...
  const handleUpdateItemGrams = (
    ...
    if (isNaN(newGrams) || newGrams <= 0) {
      toast({
        ...
        description: "Grams must be greater than zero.",
      });
      return;
    }
    ...
    const updatedLogs = logs.map((log) => {
      ...
      const { calories, protein, carbs, fat, fiber, saturatedFat, sugar } =
        recalculateNutrients(updatedItems);
  ...
  const handleAddItemToLog = async (logId: string, text: string) => {
    ...
    try {
      ...
      const updatedLogs = logs.map((log) => {
        ...
        const { calories, protein, carbs, fat, fiber, saturatedFat, sugar } =
          recalculateNutrients(combinedItems);
        const totalCalories = calories;
        const totalProtein = protein;
        const totalCarbs = carbs;
        const totalFat = fat;
      ...
      toast({
        title: "Item Added",
        description: "Successfully updated your meal record.",
      });
    } catch (error) {
      toast({
        variant: "destructive",
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to delete the record from the database.",
        });
      }
    } catch (error) {
  ...
  const activeWeeklyRange = useMemo(() => {
    ...
    return {
      from: startOfDay(subDays(weeklyPivotDate, 6)),
      to: endOfDay(weeklyPivotDate),
    };
  ...
  const dynamicWeeklyData = useMemo(() => {
    try {
      const days = eachDayOfInterval({
        start: activeWeeklyRange.from,
        end: activeWeeklyRange.to,
      });
  ...
  // Replaces 6 separate O(N) array reductions (one in weeklyAvgCalories, five in render)
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4">
                ...
                >
                  <TabsList className="bg-white/40 dark:bg-black/40 backdrop-blur-xl border border-white/60 dark:border-white/10 p-1 rounded-xl">
                    <TabsTrigger
                      value="daily"
                      className="data-[state=active]:bg-white/80 dark:data-[state=active]:bg-white/20 rounded-lg"
                    >
                      Daily
                    </TabsTrigger>
                    <TabsTrigger
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            ...
                            {format(activeWeeklyRange.to, "MMM d")}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent
                          className="w-auto p-0 rounded-2xl border-none shadow-2xl"
                              ...
                              <div className="glass-card rounded-2xl p-5 border-white/60">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="pt-2 flex justify-end">
                                    <AddItemPopover
                                      logId={log.id}
                                      onAdd={handleAddItemToLog}
                                      isAdding={isAddingItem === log.id}
                                    />
                                  </div>
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-primary/5 grid grid-cols-3 gap-y-3">
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-sm font-bold text-primary">
                                      {log.totalNutrients.saturatedFat}g
                                    </span>
                                  </div>
                                  <div className="flex flex-col">
                                    <span className="text-[10px] font-bold uppercase text-primary/60">
                          ...
                          <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                            <div className="flex items-center gap-2">
                              ...
                              {m.isLimit && isOver && (
                                <Badge
                                  variant="destructive"
                                  className="h-4 text-[8px] px-1 animate-pulse"
                                >
                                  Limit Reached
                                </Badge>
                              )}
